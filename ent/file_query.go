...
func (fq *FileQuery) QuerySource() *FileSourceQuery {
	query := (&FileSourceClient{config: fq.config}).Query()
...
func (fq *FileQuery) Clone() *FileQuery {
	...
	return &FileQuery{
		...
		withSource: fq.withSource.Clone(),
...
func (fq *FileQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*File, error) {
	...
	if query := fq.withSource; query != nil {
		if err := fq.loadSource(ctx, query, nodes, nil,
			func(n *File, e *FileSource) { n.Edges.Source = e }); err != nil {
			return nil, err
		}
	}
...
func (fq *FileQuery) loadSource(ctx context.Context, query *FileSourceQuery, nodes []*File, init func(*File), assign func(*File, *FileSource)) error {
...
func (fq *FileQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := fq.ctx.Fields; len(fields) > 0 {
		...
		if fq.withSource != nil {
			_spec.Node.AddColumnOnce(file.FieldSourceID)
		}
	}
