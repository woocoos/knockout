...
func (_q *UserAddrQuery) QueryRegion() *RegionQuery {
...
func (_q *UserAddrQuery) WithRegion(opts ...func(*RegionQuery)) *UserAddrQuery {
	query := (&RegionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserAddrQuery) loadRegion(ctx context.Context, query *RegionQuery, nodes []*UserAddr, init func(*UserAddr), assign func(*UserAddr, *Region)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*UserAddr)
	for i := range nodes {
