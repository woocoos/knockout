...
func (oruu *OrgRoleUserUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	_spec := sqlgraph.NewUpdateSpec(orgroleuser.Table, orgroleuser.Columns, sqlgraph.NewFieldSpec(orgroleuser.FieldOrgRoleID, field.TypeInt), sqlgraph.NewFieldSpec(orgroleuser.FieldOrgUserID, field.TypeInt))
...
func (oruuo *OrgRoleUserUpdateOne) sqlSave(ctx context.Context) (_node *OrgRoleUser, err error) {
	...
	_spec := sqlgraph.NewUpdateSpec(orgroleuser.Table, orgroleuser.Columns, sqlgraph.NewFieldSpec(orgroleuser.FieldOrgRoleID, field.TypeInt), sqlgraph.NewFieldSpec(orgroleuser.FieldOrgUserID, field.TypeInt))
	if id, ok := oruuo.mutation.OrgRoleID(); !ok {
		return nil, &ValidationError{Name: "org_role_id", err: errors.New(`ent: missing "OrgRoleUser.org_role_id" for update`)}
	} else {
		_spec.Node.CompositeID[0].Value = id
	}
	if id, ok := oruuo.mutation.OrgUserID(); !ok {
		return nil, &ValidationError{Name: "org_user_id", err: errors.New(`ent: missing "OrgRoleUser.org_user_id" for update`)}
	} else {
